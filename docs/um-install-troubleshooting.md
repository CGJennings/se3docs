...
If the app won't start after installing a plug-in, it is a good bet that the problem involves that plug-in. Using the instructions for catalog issues ([below](#catalogue-download-issues)), find the `plug-ins` subfolder inside your [inside your user folder](um-install-user-folder.md). Locate the just-installed plug-in(s) and delete them, then try starting the app again.

### After displaying a Fatal Error message

*Check below to see if your specific error message has its own instructions on this page.*
