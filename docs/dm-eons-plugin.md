...
This tab lets you set the most important information required by the plug-in. If the information on this tab is not correct, your plug-in will either fail to load or will not function as expected.
...
On this tab you can describe how you want the plug-in to be listed in the [catalogue](um-plugins-catalogue.md). You can provide a name, author credits, and optional home page. Beneath that you can write a longer description of the plug-in and its features. This can make use of basic HTML tags to style text or create numbered or bullet lists.

> You can provide translated versions of the catalogue entry. To do this, the list of **Interface Languages** on the **Advanced** tab must include the target language. Then you can select it from the **Language** drop down list at the bottom of the dialog. (Translatable fields are shown with a yellow background when a language other than *Default* is selected. When yellow fields are left blank, Strange Eons will fall back to whatever values are entered under the **Default** language.)

![The Catalogue Entry tab of the root editor](images/root-editor-2.png)

### Library description

The fields on this tab are only filled in for library bundles. For other bundle types, simply leave them blank. The purpose of these fields is to allow you to provide a name, representative image (icon), and description for the bundle when it is listed in the plug-in manager. (This information would normally be requested directly from the plug-in when it is loaded, but library bundles do not contain any plug-ins.)

> You can provide translated versions of the library description. To do this, the list of **Interface Languages** on the **Advanced** tab must include the target language. Then you can select it from the **Language** drop down list at the bottom of the dialog. (Translatable fields are shown with a yellow background when a language other than *Default* is selected. When yellow fields are left blank, Strange Eons will fall back to whatever values are entered under the **Default** language.)

![The Library Description tab of the root editor](images/root-editor-3.png)

### Referenced bundles

This tab lets you declare other plug-in bundles that this plug-in bundle is related to. It consists of a list of bundles for each type of relationship. The controls for each list work the same: to add a bundle, first copy its **Catalogue ID** to the clipboard, then add it to the list by choosing the paste button. To remove an entry, select it an choose the cut button. You can copy a bundle's full **Catalogue ID** text using the copy button from the **Plug-in Control** tab when editing its root file, or from its catalogue or plug-in manager listing by selecting it, right clicking on the information panel, and choosing **Show Expert Information**. Look for text matching this format:
...
![The Referenced Bundles tab of the root editor](images/root-editor-4.png)

### Advanced

The advanced tab gives you finer control over the plug-in's metadata, load order, and other details.

![The Advanced tab of the root editor](images/root-editor-5.png)
...
Adding a language to the list of interface languages makes it possible to add translations of the [catalogue entry](#catalogue-entry) and [library description](#library-description) to that language.
